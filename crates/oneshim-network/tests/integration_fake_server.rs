...
async fn fake_server_propagates_retry_after_rate_limits() {
    ...
    match error {
        CoreError::RateLimit { retry_after_secs } => assert_eq!(retry_after_secs, 7),
...
async fn fake_server_websocket_channel_times_out_on_incomplete_ack() {
    ...
    match error {
        CoreError::RequestTimeout { timeout_ms } => assert_eq!(timeout_ms, 200),
        other => panic!("unexpected error: {other}"),
    }
...
async fn websocket_session_coordinator_reconnects_after_live_channel_drop() {
    ...
    assert!(matches!(heartbeat_error, CoreError::Network(_)));
