...
impl AnnIndex for HnswAdapter {
    async fn add(&self, key: u64, vector: &[f32]) -> Result<(), CoreError> {
        ...
        tokio::task::spawn_blocking(move || -> Result<(), CoreError> {
            ...
            if cap > 0 && size > cap * 80 / 100 {
                let new_cap = cap * 2;
                idx.reserve(new_cap).map_err(|e| CoreError::InternalV2 {
            ...
            idx.add(key, &vec).map_err(|e| CoreError::InternalV2 {
                code: oneshim_core::error_codes::InternalCode::Generic,
    ...
    async fn search(&self, query: &[f32], k: usize) -> Result<Vec<(u64, f32)>, CoreError> {
        ...
        tokio::task::spawn_blocking(move || {
            let matches = idx.search(&q, k).map_err(|e| CoreError::InternalV2 {
                code: oneshim_core::error_codes::InternalCode::Generic,
    ...
    async fn remove(&self, key: u64) -> Result<(), CoreError> {
        let idx = Arc::clone(&self.index);
        tokio::task::spawn_blocking(move || -> Result<(), CoreError> {
            idx.remove(key).map_err(|e| CoreError::InternalV2 {
                code: oneshim_core::error_codes::InternalCode::Generic,
    ...
    async fn save(&self) -> Result<(), CoreError> {
        ...
        tokio::task::spawn_blocking(move || -> Result<(), CoreError> {
            ...
            let tmp_str = tmp_path.to_str().ok_or_else(|| CoreError::InternalV2 {
                code: oneshim_core::error_codes::InternalCode::Generic,
                message: "Non-UTF-8 HNSW data path".into(),
            })?;
            idx.save(tmp_str).map_err(|e| CoreError::InternalV2 {
                code: oneshim_core::error_codes::InternalCode::Generic,
                message: format!("HNSW save failed: {e}"),
            })?;
            std::fs::rename(&tmp_path, &path).map_err(|e| CoreError::InternalV2 {
                code: oneshim_core::error_codes::InternalCode::Generic,
    ...
    async fn load(&self) -> Result<(), CoreError> {
        ...
        let path = self.data_path.clone();
        tokio::task::spawn_blocking(move || -> Result<(), CoreError> {
            let path_str = path.to_str().ok_or_else(|| CoreError::InternalV2 {
                code: oneshim_core::error_codes::InternalCode::Generic,
                message: "Non-UTF-8 HNSW data path".into(),
            })?;
            idx.load(path_str).map_err(|e| CoreError::InternalV2 {
                code: oneshim_core::error_codes::InternalCode::Generic,
