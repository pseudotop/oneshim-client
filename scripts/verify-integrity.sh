...
# RUSTSEC-2026-0097 is a low-risk rand 0.7.x build-time transitive advisory
# currently reached through Tauri HTML/build tooling, not the runtime graph.
"$CARGO_CMD" audit --ignore RUSTSEC-2024-0429 --ignore RUSTSEC-2026-0097
...
find "${ROOT_DIR}/crates" "${ROOT_DIR}/src-tauri" -name 'sbom.cdx.json' -delete
...
while IFS= read -r sbom_path; do
  ...
  artifact_name="${crate_name}"
  if [ "${artifact_name}" = "src-tauri" ]; then
    artifact_name="oneshim-app"
  fi
  cp "${sbom_path}" "${ARTIFACT_DIR}/${artifact_name}.sbom.cdx.json"
  sbom_count=$((sbom_count + 1))
done < <(find "${ROOT_DIR}/crates" "${ROOT_DIR}/src-tauri" -name 'sbom.cdx.json' | sort)
