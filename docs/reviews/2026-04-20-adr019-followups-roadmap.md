...
- [x] Follow-up #1 — ✅ **COMPLETE 2026-04-20** (iter-196/197/199/201/203/204). All Tauri command signatures migrated — iter-204 milestone counted **114 across 17 files**; iter-252 re-verification shows **106 across 19 files** per `grep -cE "#\[(tauri::)?command\]"`. Whichever count is canonical, **every command-decorated function in `src-tauri/src/commands/` returns `Result<_, IpcError>`**. Infrastructure (iter-196): DTO + 11 From-chain impls + 10 Rust contract tests. Frontend TS (iter-197): IpcError interface + isIpcError + errorMessageFromInvoke + 13 Vitest tests. Migration batches: (iter-197) onboarding/detection/focus 8; (iter-199) coaching/dashboard/capture_status 26; (iter-201) settings/permissions/sync/automation 19; (iter-203) suggestions/capture/error_report/bug_report/analysis/system 35; (iter-204) audio/ai_session/integration 23 = **114/114 (100%) at iter-204**. Follow-up #3 (Frontend i18n wiring) is now unblocked.
- [🟡] Follow-up #2 — **Rust-side SHIPPED iter-206/208**; ops-side Loki pipeline + Grafana panel migration + alert-rule audit still coordinated externally. **18 high-signal scheduler-loop emission sites** now carry `err.code = %e.code()` structured field (intelligence 3 + events 4 + monitor 2 + network 7 + sync 2); CLAUDE.md documents the pattern (+adapter-error conversion recipe) so new contributors follow it. `CoreError` Display already embeds `[code]` as a fallback for panels that haven't migrated yet.
