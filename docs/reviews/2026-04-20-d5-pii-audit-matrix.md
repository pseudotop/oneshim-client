...
| 10 | `CoreError::Display` body + tracing user-text fields | R1 | 🟢 Covered (iter-16 wrapper + 3 migrations) | 16 | `oneshim-core/src/sanitized_display.rs` (new) + 3 migrated consumers | **Wrapper** (PR #457): `SanitizedDisplay<T: Display>` + `sanitized(&err, sanitizer, level)` helper in `oneshim-core`. Addition-only — `thiserror`-derived `Display` impls on 38 `CoreError` variants are intentionally preserved. **Migrations** (see Path 22): all 3 high-risk LLM-error tracing sites (`LlmWorkTypeRefiner`, `coaching_helper`, `gui_pipeline`) now route through `SanitizedDisplay`. Remaining unmigrated sites are documented-skip (storage/transport infrastructure errors — no user text). |
...
| 22 | `tracing::info!/warn!/error!` fields | R6 | 🟡 In-progress (iter-16 migrations) | 16 | various | `SanitizedDisplay` wrapper landed (PR #457). **Migrated sites**: `LlmWorkTypeRefiner` (PR #458), `coaching_helper::evaluate_and_deliver` (PR #459), `gui_pipeline::process_gui_feedback` (PR #460) — all LLM-error tracing sites where the response body may echo prompt-embedded user context. **Intentionally skipped (no PII)**: `scheduler/loops/network.rs:{35,40,55,60,63,78,124}` — these 7 sites log `CoreError::Storage`/`NetworkError::*` carrying SQLite driver errors, HTTP status lines, or internal transport messages that do NOT contain user text. Per the contract ("sanitize at ingest boundaries, trust in storage"), migrating low-risk infrastructure-error tracing adds maintenance cost without marginal privacy gain. Canonical pattern for new sites: `warn!(err.code = %e.code(), "task failed: {}", sanitized(&err, &*san, level))`. |
...
- iter-16: Paths 10 + 22 (error Display + tracing) — **complete**. `SanitizedDisplay<T>` wrapper shipped (PR #457) + 3 LLM-error migration PRs: #458 (`LlmWorkTypeRefiner`), #459 (`coaching_helper`), #460 (`gui_pipeline`). 7 `network.rs` storage-error sites intentionally skipped per documented-skip rationale (see Path 22).
