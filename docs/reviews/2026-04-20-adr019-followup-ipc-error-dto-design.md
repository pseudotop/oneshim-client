...
> **Actual scope at ship:** **114/114** command signatures migrated across **17** files per the iter-204 milestone note (not 112 — the real scan at iter-204 revealed 114). **Current reality (as of post-audit verification iter-252): 106 commands across 19 files** per `grep -cE "#\[(tauri::)?command\]"` — the delta may reflect command removal/addition churn in iter-205~251, or the iter-204 count may have included non-`#[command]` helpers. Either way, all command-decorated functions in `src-tauri/src/commands/` currently return `Result<_, IpcError>`. Infrastructure as designed (`IpcError` struct + `From` chain impls + frontend TS type guard), plus 2 feature-gated adapter-error impls (`NetworkError` under `analysis`, `EmbeddingError` under `embedding`) that the design didn't enumerate. 10 Rust contract tests + 13 Vitest unit tests. 2 UI integration demos (`GeneralTab::SupportToolsCard` translateError-as-primary + `BugReportWizard::handleExport` translateError-as-detail-suffix) added in iter-210/212 as part of Follow-up #3.
...
~~See paired plan doc: `docs/reviews/2026-04-20-adr019-followup-ipc-error-dto-plan.md` (to be authored).~~ Execution shipped directly from this design's risk-order list (no separate plan doc authored) across 6 batches = iter-196 Rust infrastructure batch (DTO + 12 From impls + 10 Rust contract tests) + 5 command-migration batches (iter-197 also bundled the frontend TS guard — `IpcError` interface + `isIpcError` + `errorMessageFromInvoke` + 13 Vitest tests — alongside its 3-file migration batch; iter-199/201/203/204 are pure command migrations) — see post-execution banner.
