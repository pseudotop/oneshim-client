...
- **최신 `main` CI** (`fix(updater): per-PID boot_count markers ...`): 실패 — [Run 24623392589](https://github.com/pseudotop/oneshim-client/actions/runs/24623392589) (2026-04-19). 원인 (iter-225 심층 조사에서 식별): 크로스 플랫폼 `Build` 매트릭스 job 이 `resource path oneshim-sandbox-worker-<triple> doesn't exist` 로 실패 — `src-tauri/tauri.conf.json` 의 Tauri externalBin 선언이 빌드 시 stub 파일을 요구하는데 `Build` job 에는 `check` / `test` job 이 이미 가지고 있던 stub-touch 단계가 누락되어 있었음. 로그 초반의 `[ARTIFACT-DL][ERROR]` 는 red herring (continue-on-error: true + fallback stub 으로 처리됨). **`feature/error-code-phase1` 에서 수정됨** (iter-225 ci.yml Build + iter-226 release-smoke.yml + build-smoke.yml + macos-windowserver-gui-smoke.yml — 총 4개 workflow). ADR-019 PR 머지 후 main-CI green 예상.
