...
## Dependabot과 dependency update

Dependency automation은 public repo의 오픈소스 장점을 살리되, public repo가
개발 source of truth로 바뀌지 않게 운영한다.

- public Dependabot alerts와 version-update PR은 투명성을 위해 켜둔다.
- public Dependabot PR은 path-aware로 분류한다.
  - `Cargo.toml`, `Cargo.lock`, Rust source, exported workflow처럼 mirror되는
    source/dependency path는 parent/client SSOT tree에 재현하고 private/full
    CI를 통과한 뒤 public export로 되돌린다.
  - repository metadata, issue template, public overlay처럼 public-only path는
    public에서 직접 처리할 수 있다. 계속 유지할 변경이면 export tooling에도
    반영한다.
  - 긴급 security exception은 maintainer 판단으로 public에 먼저 반영할 수
    있지만, 즉시 SSOT replay를 예약해야 한다.
- client가 parent repository로 승격되면 parent-side Dependabot은
  `/clients/maekon-client` 같은 client path를 대상으로 실행하고, public repo
  Dependabot config는 public visibility를 위해 `/` 기준으로 유지한다.
- 내부 auto-merge workflow는 public-minimal export에서 제외한다. mirror된
  dependency path에는 public auto-merge를 켜지 않는다.

이 기준은 `pseudotop/maekon-client`를 trust, contribution, vulnerability
visibility에 유용하게 유지하면서도 canonical implementation path는 하나로
보존한다.
