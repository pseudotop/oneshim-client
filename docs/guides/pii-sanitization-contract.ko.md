...
**영속화 또는 전송 경계를 넘는 모든 텍스트 값은 반드시 살균되어야 한다.**

"경계"란 데이터가 실행 중인 에이전트 프로세스의 인메모리 신뢰 도메인을 벗어나는 모든 지점:
...
1. **프로세스 내부 경계** — 예: 에이전트 프로세스 내부의 인메모리 regex matcher와 하위 요약기 사이를 흐르는 OCR 텍스트. 파이프라인 동작을 위해 원본이 어딘가 존재해야 함; 살균은 다음 영속화 / 전송 경계에서 적용.
...
- 사용자의 OCR, 클립보드, 접근성 텍스트, LLM 응답이 로컬 영속화되거나 외부 전송될 때 프라이버시 유지.
