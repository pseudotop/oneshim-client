...
1. **Tauri IPC code propagation** — ✅ **SHIPPED 2026-04-20** (iter-196/197/199/201/203/204). All Tauri command signatures (**106 commands across 19 files** per current `grep -cE "#\[(tauri::)?command\]"`; the iter-204 milestone-note counted 114/17) now return `Result<_, IpcError>` where `IpcError { code: String, message: String }` surfaces the ADR-019 wire code to the frontend. Foundation in `src-tauri/src/ipc_error.rs` (**12** From-chain impls + 10 Rust contract tests); frontend consumer in `crates/oneshim-web/frontend/src/api/desktop.ts` (`IpcError` interface + `isIpcError` type guard + `errorMessageFromInvoke` fallback helper + 13 Vitest unit tests). See [ipc-error-dto-design](../reviews/2026-04-20-adr019-followup-ipc-error-dto-design.md) for the original design; executed via 6 phased batches in the risk-ordered plan.
