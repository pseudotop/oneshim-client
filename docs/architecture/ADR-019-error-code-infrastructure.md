...
2. **Grafana dashboard relabeling** — 🟡 **Rust-side SHIPPED iter-206**; ops-side migration coordinated externally. 9 high-signal scheduler-loop emission sites now carry `err.code = %e.code()` as a structured tracing field (automatically surfaces as an OTel span attribute via `tracing-opentelemetry`). `CoreError` Display embeds `[code]` as a fallback so Loki can parse it either way during the migration window. CLAUDE.md documents the pattern + adapter-error conversion recipe. Remaining work (Loki pipeline config + panel migration + alert-rule audit) lives in ops infra, not client-rust. See [grafana-relabeling-design](../reviews/2026-04-20-adr019-followup-grafana-relabeling-design.md) for the original design.
