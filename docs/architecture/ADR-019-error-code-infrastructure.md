...
1. **Tauri IPC code propagation** — ⏳ **Infrastructure shipped iter-196**. `src-tauri/src/ipc_error.rs` now exports `IpcError { code, message }` + 11 `From` chain impls (`CoreError`, `GuiInteractionError`, 6 unconditional adapters + 2 feature-gated + 2 stdlib) + 10 contract tests. Remaining work: migrate 112 command signatures from `Result<_, String>` to `Result<_, IpcError>`, phased low-risk → state-mutating → streaming. See [ipc-error-dto-design](../reviews/2026-04-20-adr019-followup-ipc-error-dto-design.md) for the migration plan.
