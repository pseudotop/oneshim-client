...
## Phase 9 PR-B2: Autostart Linux Deep (v0.4.41-rc.1, 2026-04-27)

- **systemd Type=notify integration**: replaced PR-B1's `Type=simple` with `Type=notify` + `NotifyAccess=main` + `TimeoutStartSec=30` in `linux::generate_service_file()`. New `lifecycle::sd_notify::notify_ready()` wrapper sends READY signal at end of `setup::init()` (last line before `Ok(())`). Non-default `systemd-notify` Cargo feature flag required for Linux release builds (`--features systemd-notify`).
- **Hash-based deferred migration**: `KNOWN_PRIOR_HASHES` registry in `lifecycle/migration_hashes.rs` covers BOTH PR-B1 brand variants — v0.4.40-rc.1 / v0.4.40-rc.2 (`Description=ONESHIM Desktop Agent`, hash `beebd724...`) AND v0.4.40-rc.3 / v0.4.40 stable (`Description=Maekon Desktop Agent`, hash `9b1f5d38...`). `canonicalize()` normalizes line endings (`\r\n` → `\n`) and word-boundary-replaces ExecStart line. `run_startup_migration()` (Linux-only, called from `setup::init()` after D-Bus check) detects existing PR-B1 service files and overwrites with PR-B2 template; defers `daemon-reload` to next user login (no service interruption). Customized files skipped with warn log.
- **Real `detect_capabilities()` Linux env detection**: replaces PR-B1 skeleton at `autostart.rs:495`. 5-step priority chain: SNAP env → `LinuxSnapSandbox` (supported=false), FLATPAK_ID env → `LinuxFlatpakSandbox` (false), no DISPLAY/WAYLAND_DISPLAY → `LinuxHeadless` (false), `linux::has_systemctl()` → `LinuxSystemd` (true), else → `LinuxXdg` (true).
- **4 new wire codes** (per ADR-019): `autostart.{sd_notify_skipped, service_migrated, service_migration_failed, service_migration_skipped}`. Wire snapshot 49 → 53. Atomic commit (Tasks 2+3 combined per `wire-error-i18n-coverage` lefthook hook). Vitest `translateError.test.ts` count assertions bumped 49 → 53 (4 places).
- **Linux integration tests**: rootless approach per Phase 1 review. Two-job split: `linux-autostart-unit` always-on CI job (`.github/workflows/ci.yml`) runs smoke + 3 inline capability tests + lifecycle tests under `--features systemd-notify`; `linux-systemd-integration.yml` workflow_dispatch for manual T8-T10 (modifies user systemd state).
- **Korean operations guide**: `docs/guides/autostart.ko.md` (~265 lines) covers platform behavior matrix, environment-specific support (systemd 219+ minimum noted), automatic + manual migration steps for both ONESHIM and Maekon brand eras, troubleshooting (systemd timeouts, customized service files, headless degradation), single-instance limitations.
- **Tests**: +12 macOS-host Rust unit tests (`lifecycle::migration_hashes` 10 + `lifecycle::sd_notify` 2). Linux-only inline tests (`autostart::tests::linux_capability_tests` +3 SNAP/FLATPAK/headless via `#[serial]`, `linux_autostart_unit` smoke +1) compile but execute only on Linux CI. Vitest unchanged (wire-error count assertion bumped 49 → 53). 3 manual integration tests (`#[ignore]`, T8/T9/T10 in `linux_autostart_systemd_live.rs`) for workflow_dispatch.
- **Workspace totals**: cargo test `3,906 passed, 0 failed, 24 ignored` (macOS host; +12 over PR-B1 baseline of 3,798 v0.4.39-rc.1). Vitest `279 passed / 43 test files`.
- **Spec + plan**: `docs/superpowers/specs/2026-04-25-phase9-pr-b2-autostart-linux-deep-design.md` (v2.5) + `docs/superpowers/plans/2026-04-25-phase9-pr-b2-autostart-linux-deep-plan.md` (v2). Plan adjustments at execution time: counts 47→51 corrected to 49→53 (TimeWindow primitive added 2 codes between plan write and impl); `Description=ONESHIM` corrected to `Description=Maekon` (Maekon rebrand #520 between plan write and impl, both brand hashes registered in migration registry).
- **Implementation**: 14 commits on branch `feature/phase9-autostart-linux-deep` (Tasks 1-15 with combined Task 2+3 atomic commit + Task 7 fix for missing ONESHIM hash + i18n consistency fixes), reset onto post-PR-B1 main `0613f976`.
