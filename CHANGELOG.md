...
- Autostart IPC commands + settings UI (P3)
  Wire existing autostart functions to Tauri IPC and add a "Start on login"
  toggle in Settings > General, visible only when running inside Tauri.

- Wayland idle detection improvements (P4)
  Add KDE ScreenSaver D-Bus and systemd-logind idle detection to the
  Wayland fallback chain, expanding coverage beyond GNOME Mutter.

- Linux AT-SPI element caching (P1)
  Add an LRU cache for AT-SPI accessibility element trees to avoid
  re-querying the full tree on every frame capture. Keyed on (pid,
  title_hash) with configurable TTL (default 5s).
