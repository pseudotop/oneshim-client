...
- **ADR-019** Error Code Infrastructure + C5 AWS Bedrock intentional non-support. Every struct-variant of `CoreError` (28 struct + 2 `#[from]`-wrapped = 30 total) and `GuiInteractionError` (8 variants) carries a typed `code: XxxCode` field; the 2 `#[from]` variants derive their code via `impl CoreError::code()` per ADR-019 §7. `err.code() -> &'static str` gives a single wire-format entry point for Grafana/logs/i18n. 18 typed code enums under `crates/oneshim-core/src/error_codes/` via a single-source `define_code_enum!` macro. Wire-format contract locked via `crates/oneshim-core/tests/wire_contract_snapshot.rs` (54 codes). AWS Bedrock deleted from `specs/providers/provider-surface-catalog.json` with 8 match arms in `oneshim-network` + 2 defense-in-depth guards (`analysis_client`, `ai_model_catalog_web_service`) returning `ConfigCode::UnsupportedProviderBedrock` — closes the silent no-auth fallthrough security bug in OCR `apply_auth_headers` plus 3 additional Bedrock defects found during post-merge drift audit (2 silent-fallthrough sibling paths + 1 wildcard-mislabel in `http_api_session::build_request_body`). See [ADR-019](docs/architecture/ADR-019-error-code-infrastructure.md) (with Korean companion). ~1,030 callsites retrofitted across 13 crates via 4-phase soft-migration (V1→V2→V1-rename) landed as a single branch. **Post-merge drift audit (iter 87~172)** further removed 16 orphan wire codes (iter-87 × 3 + iter-148 `gui.generic` + iter-161 × 11 unused `*Code::Generic` placeholders: audio/config/consent/oauth/permission/policy/provider/secret/service/storage/validation + iter-163 `auth.generic` — all migration-era boilerplate with zero production emission sites) + 2 dead `CoreError` variants pre-merge per YAGNI, re-routed ~138 semantically-drifted `Internal.Generic` emissions to specific variants (Config.Missing/Invalid/OutOfRange, NotFound, ServiceUnavailable, InvalidArguments, Analysis, OcrError), added ~38 regression tests covering the corrected mappings, brought 15 HTTP dispatchers (up from 14) to the canonical semantic status-mapping pattern per [`docs/guides/http-status-error-mapping.md`](docs/guides/http-status-error-mapping.md), completed a **port trait `# Errors` doc standardization campaign** (62/62 ports across `crates/oneshim-core/src/ports/` now carry canonical `# Errors` sections; iter-154 fixed 9 stale wire-code references — `platform.permission_denied`→`permission.permission_denied` × 2, `network.connection_failed`→`network.generic` × 7; iter-155 fixed 3 port-doc-vs-impl inconsistencies — `ModelDownloader::delete_model` documented as emitting `CoreError::Io` but actually emits `AudioCapture`, `OverrideStore::delete_override` documented as rowcount=0 Ok(()) but actually emits `NotFound { resource_type: "RegimeOverride" }`, and `CoachingStoragePort::update_coaching_event_personalized` tightened from "may return Ok(()) or NotFound" to definitively Ok(()); iter-156 fixed another drift — `focus_storage` port doc incorrectly grouped `end_work_session` with rowcount=0 no-op methods, but `SqliteStorage::end_work_session` uses `RETURNING duration_secs` so unknown `session_id` surfaces as `CoreError::Storage` via `QueryReturnedNoRows` (other methods in the no-op group verified + weasel language "typically" tightened on session_storage + few_shot_storage); iter-157 corrected stale adapter struct name in `notifier` port doc — `DesktopNotifierImpl` doesn't exist; real adapters are `TauriNotifier`/`LogOnlyNotifier` in `src-tauri/src/agent_runtime_support.rs` (systematic sanity-check of every "Implemented by X" claim in port docs verified 18 other named adapters exist); iter-158 `credential_source` port doc drift — claimed `ConfigCode::Missing` for secret-store-not-initialized but impl emits `ConfigCode::Invalid`; iter-159 documented latent `element_finder` default-impl match gap (first default message "does not support scene analysis" doesn't match any substring in the caller's BadRequest routing pattern — currently harmless because all production adapters override); iter-160 fixed 3 stale ADR-019 sub-section references (`oauth.rs` port doc `§3.5`, `error.rs` comment `§7.5`, ADR-019 Known follow-up #6 `§12` — ADR-019 has §1-7 only with no sub-subsections; refs redirected to the real locations: `token_exchange.rs::classify_error_response` for RFC 6749 classifier, §1 for wire-format immutability, `http-status-error-mapping.md` for HTTP status pattern registry)), re-routed a further **41 SQLite adapter `Internal.Generic` → `Storage.failed`** emissions in `web_storage_impl.rs`/`integration_query_impl.rs`/`fts_search_impl.rs`/`calibration_store_impl.rs` to match the iter-47 mass fix pattern already used by preset/annotation/few_shot adapters, re-routed **10 subprocess-provider callsites** (`classify_subprocess_error`) so LLM subprocess exit failures surface as `CoreError::Analysis` and OCR subprocess exit failures as `CoreError::OcrError` instead of sharing `Internal.Generic` (iter-149 added `SubprocessKind { Llm, Ocr }` discriminator), re-routed **16 more sites** across `ai_llm_client/` + `ai_ocr_client/` + `http_api_session/` + `subprocess_provider/runtime.rs` (iter-150/151/152/153) so LLM response-envelope extraction failures emit `Analysis` / `provider.analysis_failed` and catalog lookup faults (`resolved_request_shape` / `resolved_auth_scheme` / `validate_supported_parameters` / `subprocess_invocation_mode` / shape-role mismatch) emit `Config.Invalid` / `config.invalid`, and eliminated a parallel-test flake (`circuit_breaker_skips_when_tripped`) via `serial_test` on module-level `CONSECUTIVE_TIMEOUTS` mutation. Snapshot shrank 54 → 41 wire codes across iter-148 + iter-161 + iter-163. iter-162 updated the ADR-019 "Post-merge orphan wire-code cleanup" section (EN + KO) to record iter-148/161 continuation with retained-variants detail, so future readers don't assume the 57→54 delta is the current state. iter-163 completed the Generic cleanup by removing `auth.generic` — post-iter-161 re-audit revealed its only emission was a test-only mock (`TestSessionPort` in integration handler tests) which was switched to `AuthCode::Failed` (semantically correct for "not authenticated"). Generic variants now retained (2): `internal.generic` (workspace-wide Internal fallback), `network.generic` (HTTP status fallback). iter-164 extended the YAGNI audit from wire codes to adapter error types: `NetworkError` had 5 dead variants (`Serialization`, `OAuth`, `OAuthRefresh`, `Ocr`, `SecretStore`) with zero construction sites — the match arms in `impl From<NetworkError> for CoreError` were literally unreachable. Removed the 5 variants + match arms, leaving 12 active NetworkError variants (was 17). `StorageError::Core` and `StorageError::Io` verified as legitimately retained despite 0 explicit constructions because they're `#[from]`-wrapped and used via `?`-propagation across 198 functions returning StorageError. iter-165 continued the adapter error audit across 4 more enums: removed 10 dead variants total — `AutomationError` × 5 (`Config`, `Io` (#[from] but no ?-propagation usage in automation), `PrivacyDenied`, `SandboxUnsupported`, `ServiceUnavailable`; retained `UserDenied`/`PolicyBlocked` unit variants because a regression-guard test documents their canonical `policy.denied` mapping), `VisionError` × 2 (`PermissionDenied`, `ElementNotFound`), `AnalysisError` × 2 (`Internal`, `LlmService`), `SuggestionError` × 1 (`Internal`). All `#[from] Core` variants retained across all adapter types as future escape hatch for CoreError composition. iter-168 closed the final docs-drift vector from iter-164/165: the historical plan `docs/superpowers/plans/2026-03-31-error-strategy-compliance.md` retained code examples using the removed variant names. Added a `Status: COMPLETED (with post-merge cleanup)` banner at the top of that plan pointing to ADR-019 §Post-merge orphan wire-code cleanup so readers understand the variants shown in the plan snippets no longer exist in the current source tree. iter-169 synced `docs/STATUS.md`+`STATUS.ko.md` (single source of truth per CLAUDE.md) — EN was stale at 3,455 tests, KO more stale at 2,995 tests + v0.4.21 + schema V25; both synced to current 3,641/0/21 + v0.4.39-rc.1 + V31. iter-170 recorded iter-164/165 adapter-error YAGNI in the ADR-019 "Post-merge orphan wire-code cleanup" section (EN + KO) since it previously stopped at iter-163. iter-171 fixed stale ADR-001 §1 NetworkError example that showed `Http(#[from] reqwest::Error)` + a non-existent `Sse(String)` variant; updated to match current 13-variant state with pointer to `crates/oneshim-network/src/error.rs` as canonical source + cross-reference to ADR-019. iter-172 fixed pre-ADR-019 `CoreError` constructor syntax drift in ADR-008 (`CoreError::RateLimit { retry_after_secs }` missing typed `code` field — added top-of-document note following ADR-007 precedent) and ADR-011 (`CoreError::Analysis(String)` → struct variant inline). No source code change.
