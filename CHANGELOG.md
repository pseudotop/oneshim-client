...
- **ADR-019** Error Code Infrastructure + C5 AWS Bedrock intentional non-support. Every struct-variant of `CoreError` (30 struct + 2 `#[from]`-wrapped = 32 total) and `GuiInteractionError` (8 variants) carries a typed `code: XxxCode` field; the 2 `#[from]` variants derive their code via `impl CoreError::code()` per ADR-019 §7. `err.code() -> &'static str` gives a single wire-format entry point for Grafana/logs/i18n. 19 typed code enums under `crates/oneshim-core/src/error_codes/` via a single-source `define_code_enum!` macro. Wire-format contract locked via `crates/oneshim-core/tests/wire_contract_snapshot.rs` (57 codes). AWS Bedrock deleted from `specs/providers/provider-surface-catalog.json` with 8 match arms in `oneshim-network` + 2 defense-in-depth guards (`analysis_client`, `ai_model_catalog_web_service`) returning `ConfigCode::UnsupportedProviderBedrock` — closes the silent no-auth fallthrough security bug in OCR `apply_auth_headers` plus 3 additional Bedrock defects found during post-merge drift audit (2 silent-fallthrough sibling paths + 1 wildcard-mislabel in `http_api_session::build_request_body`). See [ADR-019](docs/architecture/ADR-019-error-code-infrastructure.md) (with Korean companion). ~1,030 callsites retrofitted across 13 crates via 4-phase soft-migration (V1→V2→V1-rename) landed as a single branch.
