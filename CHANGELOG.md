...
- **ADR-019** Error Code Infrastructure + C5 AWS Bedrock intentional non-support. See [ADR-019](docs/architecture/ADR-019-error-code-infrastructure.md) (with Korean companion).
  ...
  - **Post-merge drift audit (iter 87~214)** — summary; per-iter detail lives in ADR-019 §Post-merge orphan wire-code cleanup and the git log.
    ...
    - **HTTP status mapping standardization**: 15 dispatchers brought to the canonical pattern per [`docs/guides/http-status-error-mapping.md`](docs/guides/http-status-error-mapping.md); ~38 regression tests added.
    ...
    - **Doc sync**: ADR-019 EN+KO synced through iter-165 adapter-error YAGNI; ADR-001 NetworkError example updated to match current 13-variant enum; ADR-007/008 carry pre-ADR-019 syntax notes; ADR-011 `CoreError::Analysis(String)` updated inline to struct variant; `STATUS.{md,ko.md}` synced (EN 3,455 → 3,641; KO 2,995 → 3,641 + v0.4.21 → v0.4.39-rc.1 + schema V25 → V31); historical plan `docs/superpowers/plans/2026-03-31-error-strategy-compliance.md` got a "COMPLETED (with post-merge cleanup)" banner flagging removed-variant code examples; 4 additional phase implementation plans/specs (`2026-04-17-phase2-config-telemetry-plan`, `2026-04-18-phase3-regime-feedback-learning-{spec,plan}`, `2026-04-18-phase5-d8-storage-test-backfill-spec`) got SHIPPED + pre-ADR-019 syntax banners. iter-175 converged accidental duplicate `docs/plans/` directory into canonical `docs/plan/` (6 files renamed via `git mv`; 1 internal self-reference path updated; empty `plans/` dir removed) per the `DOCUMENTATION_POLICY.md` + `docs/plan/README.md` canonical convention. iter-176/177 synced `docs/README.{md,ko.md}` + `docs/DOCUMENTATION_POLICY.{md,ko.md}` with 5 missing directory entries (`reviews/`, `roadmap/`, `specs/`, `testing/`, `superpowers/`) totaling 60+ tracked files previously invisible to the doc index. iter-178 fixed a high-value new-contributor drift in `CONTRIBUTING.md` — the mock ApiClient example returned `CoreError::Network("test failure".to_string())` (pre-ADR-019 tuple syntax that no longer compiles); updated to current struct-variant form. iter-179 backfilled `docs/PHASE-HISTORY.md` (CLAUDE.md references it as the authoritative phase digest, but it stopped at Phase 40 / v0.1.7) with 5 missing phase entries: Phase 2 Config Telemetry, Phase 3 Regime Feedback Learning, Phase 4 Updater Hardening (D9/D10/D11), Phase 5-D8 Storage Test Backfill, and ADR-019 Error Code Infrastructure + C5 Bedrock Skip. iter-180 fixed two drifts in `docs/release-checklist.md`: removed a reference to a "Nightly suite" workflow that no longer exists (replaced with `release-smoke.yml`), and changed the Sign-off instruction from `git tag -s v{VERSION}` (which bypasses CHANGELOG+Cargo.toml+tauri.conf.json sync) to the mandatory `./scripts/release.sh <VERSION>` flow per the CLAUDE.md Release Process section. iter-181 corrected two drifts in `AGENTS.md`: "DDD + Hexagonal Architecture" mischaracterization (client-rust is Hexagonal-only; DDD is a parent-project server concern) and the stale "oneshim-app is the composition root" claim (the former `crates/oneshim-app/` was removed from the workspace — composition root is now `src-tauri/` with `oneshim-app` as its package name). iter-182 fixed the CLAUDE.md workspace-structure drift: the crate tree listed a non-existent `crates/oneshim-app/` (even marked DEPRECATED) while omitting the active `oneshim-sandbox-worker` crate (out-of-process sandboxed automation executor, stdin JSON → stdout JSON under platform sandbox); updated the tree, added a Crate Summary entry, and added the dependency-diagram node. iter-183 applied the same sandbox-worker + package-count fix to `docs/crates/README.md` (referenced from AGENTS.md as the authoritative crate reference — "15-package workspace" via `cargo metadata --no-deps`, not the stale "14-package / 13 under crates/" count). iter-184 fixed two more CLAUDE.md numerical drifts: "9-loop background scheduler" (actual 16 loops — listed all by name) and "ADR-001~ADR-004" (actual ADR-001~ADR-019). iter-185 applied the same loop-count/ADR-range/missing-crates fix to `README.md` (EN) and the ADR-range-only fix to `README.zh-CN.md` + `README.es.md`. iter-186 performed a cross-ADR audit: unified status terminology (6 "Approved" → "Accepted" to match Nygard convention across ADR-001/014-018 + their Korean companions), promoted 3 implemented-but-still-Proposed ADRs to Accepted with implementation-evidence notes (ADR-002 OS GUI Boundary M3, ADR-007 Async Runtime Safety, ADR-008 Network Resilience), and updated ADR-002's scope to remove the stale `oneshim-ui` reference (crate removed by ADR-004 Tauri migration) with proper pointer to `MagicOverlayDriver` as the WebView bridge replacement. Conflict/overlap review of all 19 ADRs found zero consolidation candidates — each ADR has a distinct scope and ADRs 009/019 explicitly declare their non-replacement relationship with ADR-001/003. iter-187 propagated the ADR-002 scope update into the body (line 125 overlay-location sentence + Target Responsibility Map adding an `oneshim-vision` row + `src-tauri (pkg oneshim-app)` with `MagicOverlayDriver`) + KO companion. iter-188 clarified ADR-009 Scope — the `oneshim-app` package now lives in `src-tauri/` after ADR-004 Tauri migration; former `crates/oneshim-app/` directory is gone. iter-189 fixed CLAUDE.md counts — "14 HTTP dispatchers" → 15 (iter-98 `auth::refresh` added) and "29 REST endpoints" → "~30 REST endpoints" with section breakdown. iter-190 fixed `scripts/generate-app-icons.sh` default OUTPUT_DIR from the removed `crates/oneshim-app/assets` to the current `src-tauri/icons`. iter-191 added `Status: SHIPPED` banners to 6 more historical phase review docs (phase2-config-telemetry-spec, phase4-updater-hardening-design/plan, phase5-d8-storage-test-backfill-plan, wave1-phase4-followups-design/plan). iter-192 authored 4 ADR-019 Known Follow-up design docs + aggregating roadmap: (1) Tauri IPC `IpcError { code, message }` DTO with `From<CoreError>` chain impls and phased migration of 112 command signatures + 106 `.to_string()` callsites + frontend TS type guard, (2) Grafana `err_code` indexed-label relabeling with Loki pipeline stage + panel/alert migration, (3) Frontend i18n wiring with 41-key en/ko translation resources + build-time coverage check against `wire_contract_snapshot.expected.txt` (hard dependency on #1), (5) LAN transport auth regression tests via `rcgen` + `tokio_rustls::TlsAcceptor` fixture covering 5 canonical status classes (401/429/503/504/500); #4 `InternalCode` granularity is flagged as evergreen driven by the #2 telemetry signal. iter-193 wired the ADR-019 §Known follow-ups section (EN + KO) to link each item to its authored design doc and corrected the #1 callsite count (~58 → ~106) + effort (~0.5 day → ~1.5 day) against the real scan. iter-194 standardized ADR writing conventions — added `docs/architecture/ADR-TEMPLATE.md` (+ Korean companion) with the canonical header order + required sections + status keyword state machine + filename convention + usage instructions, and `docs/architecture/README.md` (+ KO) with a registry table of all 19 ADRs + "next available ID: ADR-020" + per-directory relationship summary. Before this, 19 ADRs used 5 different header styles (`**Status`/`**Date`/`**Scope` majority; `**Date`/`**Status`/`**Deciders`/`**Related` on ADR-004/005/006; inline no-bold on ADR-010; bullet list on ADR-019; table format on ADR-011/012/013) and no template existed. iter-195 shipped ADR-019 Known Follow-up #6 (`sync/lan_transport::authenticate_with_peer` regression tests): refactored the inline `match status.as_u16()` block in `crates/oneshim-network/src/sync/lan_transport/auth.rs` into a private `map_challenge_status_to_error(status_code, peer_id) -> CoreError` helper and added 6 unit tests (5 canonical statuses 401/403/429/503/504 + 1 500-fallback). Chose this pure-function-extraction approach over the original design's rustls-TlsAcceptor fixture — same coverage, no dev-dep additions, sub-millisecond per test. 15/15 HTTP dispatchers now have both Impl ✓ and Tests ✓; registry row in `docs/guides/http-status-error-mapping.md` updated accordingly. iter-196 shipped ADR-019 Known Follow-up #1 infrastructure: `src-tauri/src/ipc_error.rs` with `IpcError { code, message }` DTO + 11 `From` chain impls (`CoreError` / `GuiInteractionError` / 6 unconditional adapter errors — `StorageError`, `AutomationError`, `VisionError`, `AnalysisError`, `SuggestionError`, `MonitorError` / 2 feature-gated — `NetworkError` under `analysis`, `EmbeddingError` under `embedding` / 2 stdlib — `io::Error`, `serde_json::Error`) + 10 contract tests covering wire-code preservation, `#[from]` variant codes (internal.serialization + internal.io), Bedrock code, GuiInteractionError, adapter-chain hop, direct construction, and the 2-field serialization-shape guard. Workspace tests: 3641 → 3651 (+10). The 112 command-signature migrations from `Result<_, String>` → `Result<_, IpcError>` follow in subsequent PRs per the staged risk-ordered plan in the design doc. iter-197 began the migration — 3 lowest-risk command files (`onboarding.rs` 3 signatures, `detection.rs` 2 signatures including one `IpcError::new("validation.invalid_arguments", ...)` for a precondition-violation path, `focus.rs` 2 signatures) migrated, 8/112 complete (~7%). Added `crates/oneshim-web/frontend/src/api/desktop.ts` with the `IpcError` TS interface, `isIpcError` type guard, and `errorMessageFromInvoke` fallback helper (tolerant of the mid-migration window where some commands still surface plain strings) + 13 Vitest unit tests for the guard's narrowing/fallback behavior. Removed the iter-196 `#[allow(dead_code)]` annotations on `IpcError` struct + `::new()` now that real callers exist. iter-199 continued the read-only bucket migration with 3 more command files — `coaching.rs` (12 signatures; direct `IpcError::new` constructions for invalid dismiss action / invalid overlay mode → `validation.invalid_arguments`, overlay-not-initialized → `service.unavailable`), `dashboard.rs` (9 signatures, densest file so far with 24 previous `.to_string()` sites; Storage + JSON errors routed through `IpcError::from` via the chain impls, 2 `validation.invalid_arguments` for "Invalid date format"), `capture_status.rs` (9 signatures; one `IpcError::new("not_found.resource_missing", ...)` for "main window not found"). Total: 34/112 signatures migrated (~30%). Pattern observations: `.map_err(IpcError::from)` is the dominant shape when upstream errors are typed (Storage/JSON/NetworkError); `IpcError::new(<wire_code>, msg)` is used only at argument-validation / precondition-violation boundaries where no upstream error exists. iter-201 moved from the read-only bucket to the state-mutating bucket — migrated 4 more files: `settings.rs` (3 sigs; kept `validate_config_bounds`/`reject_forbidden_allowed_subpaths` private helpers returning `Result<(), String>` per their existing test substring-matching contract, with a local `validation_error()` helper wrapping at the command boundary), `permissions.rs` (3 sigs; local `permission_error()` helper mapping desktop_permissions string errors to `permission.permission_denied`), `sync.rs` (5 sigs; upstream returns `Result<_, CoreError>` so `.map_err(IpcError::from)` works directly; `sync_not_enabled()` helper for disabled-engine `service.unavailable`), `automation.rs` (7 sigs; `automation_not_available()` helper + direct `IpcError::new("not_found.resource_missing", ...)` for "preset not found via IPC"). Running total: **56/114 signatures migrated (~49%)** — crossed the halfway point. Remaining 58 signatures across audio/suggestions/system (state-mutating) and integration/ai_session/analysis/capture/bug_report/error_report/suggestion_parser (streaming / heavy-IO). iter-203 batched 6 more files: `suggestions.rs` (9 sigs — local helpers for suggestions+ai_sessions unavailability, stream-error → `provider.analysis_failed`, 60s drain timeout → `network.timeout`, session-not-found → `not_found.resource_missing`), `capture.rs` (2 sigs — frame/activity monitor missing → `service.unavailable`), `error_report.rs` (1 sig — route + severity validation → `validation.invalid_arguments`), `bug_report.rs` (1 sig — dialog task join error → `internal.generic`), `analysis.rs` (4 sigs — validate_analysis_config kept `Result<(), String>` for test compat; errors flow through `ConfigManager::update_with` → `CoreError::Config{Invalid}` → IpcError `{code: "config.invalid"}` semantically), `system.rs` (8 sigs — UpdateError has no From<UpdateError> for CoreError so preview_update uses direct `IpcError::new("internal.generic", e.to_string())`). Running total: **91/114 signatures migrated (~80%)**. Only 23 signatures remain across 3 files (audio 10, ai_session 10, integration 12). iter-204 shipped the final batch — `audio.rs` (10 sigs; local `audio_capture_not_available()` helper + download-in-progress/STT-missing → service.unavailable), `ai_session.rs` (10 sigs; session manager/storage missing → service.unavailable, daily token budget exhausted → `policy.denied` for distinct frontend handling), `integration.rs` (12 sigs; rewritten to drop 13 now-unused `|e: oneshim_core::error::CoreError| e.to_string()` type annotations — From<CoreError> inference handles it cleanly). **Follow-up #1 is now 114/114 COMPLETE** with 17 command files migrated + IpcError foundation + frontend TS type guard + 23 total tests (10 Rust contract + 13 Vitest). Follow-up #3 (Frontend i18n wiring, which had a hard dependency on #1) is now unblocked for execution. iter-205 shipped Follow-up #3: `crates/oneshim-web/frontend/src/i18n/wire-errors.{en,ko}.json` cover all 41 wire codes from the canonical `wire_contract_snapshot.expected.txt` registry; `translateError.ts` provides a graceful fallback chain (IpcError + known wire code in locale → formatted template → fallback to English → fallback to raw message → plain-string pass-through → `Error.message` → `String()`); 18 Vitest unit tests include coverage-parity assertions that read the Rust snapshot file directly, so adding a new wire code without en+ko translations fails the frontend test suite; `scripts/check-wire-error-i18n-coverage.sh` is the fast-fail pre-test CI guard. Piecemeal component migration of existing `.catch((err) => err.message)` sites to `.catch((err) => translateError(err, locale))` can now proceed without worrying about Rust ↔ TypeScript wire-code drift. **3/5 ADR-019 Known Follow-ups now shipped** (#1 Tauri IPC, #3 Frontend i18n, #5 LAN transport tests); #2 Grafana relabeling remains designed-but-not-executed (ops coordination required), and #4 Internal granularity is evergreen. iter-206 shipped the client-rust portion of Follow-up #2 (Grafana relabeling): 9 high-signal scheduler-loop tracing emission sites now carry an `err.code = %e.code()` structured field (tracing-opentelemetry bridges this into OTel span attributes automatically), covering intelligence loop server-coexistence/suggestion-save/analysis-failure, events loop process/input/clipboard/file-access save failures, and monitor loop window/context event save failures. CLAUDE.md §Coding Conventions now documents the canonical pattern (`warn!(err.code = %e.code(), "...: {e}")`) plus the adapter-error conversion recipe (`let core: CoreError = e.into();`) so future contributors follow it automatically. `CoreError` Display embeds `[code]` already, so Loki regex extraction works as a fallback during the panel-migration window. Ops-side work (Loki pipeline stage + Grafana panel migration + alert-rule audit) remains coordinated externally — it lives in the ops infrastructure repo, not client-rust. All 5 ADR-019 Known Follow-ups have now been addressed to the extent client-rust can: #1 shipped, #2 Rust-side shipped (ops-side pending external), #3 shipped, #4 evergreen (driven by #2 telemetry), #5 shipped. iter-208 expanded `err.code` structured-field coverage from 9 → 16 scheduler-loop emission sites by adding 6 sites in `network.rs` (batch-upload mark-sent / batch-upload flush error / event+frame retention × 3 / shutdown-flush error / heartbeat) and 1 site in `sync.rs` (cross-device sync cycle failure + shutdown sync push failure share the variable). Design doc's "20-30 high-value sites" target is in range; remaining lower-signal sites are left for opportunistic addition driven by specific ops panel needs rather than batch migration. iter-210 demonstrated the Follow-up #3 component-integration pattern on the first UI site: `src/pages/setting-tabs/GeneralTab.tsx::SupportToolsCard` now routes `get_runtime_log_snapshot` catch handler through `translateError(error, locale)` so IpcError emissions get localized user-facing text from the wire-errors JSON. Locale is derived from `i18n.language` via `useTranslation()`. Existing `t('settings.supportLogsLoadFailed')` kept as a defensive fallback. Pattern is the 3-step recipe (add import / destructure `i18n` / call `translateError` in `.catch`) other `.catch((err) => err.message)` sites can adopt piecemeal. Biome auto-fixes (format + import organization) also landed in this commit. iter-212 added a second UI integration (`BugReportWizard::handleExport`) with a different composition variant — suffix-append pattern: `${t('bugReportExportFailed')}: ${translateError(error, locale)}` — so users see "Failed to export: Invalid arguments: bundle json too large" instead of just "Failed to export." Two migration patterns now documented: translateError-as-primary (iter-210) and translateError-as-detail-suffix (iter-212). Future .catch sites can pick whichever fits. iter-214 wired `scripts/check-wire-error-i18n-coverage.sh` into `lefthook.yml` as a `pre-commit` hook gated by the 3 source-of-truth files (`wire_contract_snapshot.expected.txt` + `wire-errors.{en,ko}.json`) — provides ~3ms fast-fail coverage check for local dev in addition to the Vitest snapshot-driven parity tests that run via `pnpm test` in CI. Three-way source-of-truth parity confirmed in the final holistic review: Rust snapshot = EN keys = KO keys = 41, sets identical.
