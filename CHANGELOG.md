...
- Show main window on first launch
  Change visible: false → true so the dashboard window appears
  immediately after installation. Users no longer need to know about
  the menu bar icon to start using the app.

  The Reopen handler still works for re-showing after close.

- Comprehensive lint enforcement — GritQL + CI gate script

- Active-window-aware multi-monitor capture
  Single monitor: captures primary as before.
  Multi-monitor: identifies which monitor contains the active window
  (via center-point hit test) and captures that monitor's full screen.
  Falls back to primary when bounds are unavailable or no monitor
  matches.

  - Add `capture_for_window(bounds)` to ScreenCapture
  - Add `window_bounds` field to CaptureRequest
  - Wire window bounds from scheduler into CaptureRequest
  - Remove redundant window_bounds parameter from handle_frame_capture

- Add IndicatorConfig for tracking overlay preferences

- Pause/resume state + IPC commands + scheduler pause check
  - Add capture_paused and indicator_visible AtomicBool fields to AppState
  - Create capture_status IPC commands: get_capture_status, toggle_capture_pause,
    set_indicator_visible — emits overlay:capture-state-changed events
  - Wire capture_paused through AgentRuntimeBuilder → AgentRuntimeBundle → Scheduler
  - Monitor loop skips capture/frame processing when capture_paused is true
  - Register 3 new Tauri invoke commands

- Tray status items + capture toggle shortcut + tracking panel window
  Tray menu (Task 4):
  - Add capture status display, pause/resume toggle, and indicator visibility
    toggle as the first three items in the system tray menu
  - Set explicit tray ID ("main-tray") via TrayIconBuilder::with_id for
    runtime tray access
  - Add rebuild_tray_menu() that recreates the full menu with updated
    status text (Tauri v2 menu items are immutable after creation)
  - Both toggle handlers emit overlay:capture-state-changed to magic-overlay
    and tracking-panel windows

  Global shortcut + panel window (Task 5):
  - Register CmdOrCtrl+Shift+\ as capture pause toggle shortcut
  - Add create_tracking_panel() in magic_overlay.rs — 220x36 transparent,
    always-on-top, undecorated window centered at top of primary monitor
  - Create tracking-panel.json capability with core:default + event + window
    permissions
  - Call create_tracking_panel() during setup after desktop startup

- Tracking border overlay + panel entry point with pause/hide controls

- Tracking indicator integration + validation fixes
  Update Cargo.lock for 0.4.0 version alignment and disable
  beforeDevCommand to prevent auto-starting pnpm dev during tauri dev.

  All validations pass:
  - cargo check --workspace: OK
  - cargo test --workspace: 2455 passed, 0 failed
  - pnpm build: OK (43 chunks)
  - pnpm test: 120 passed (29 files)
  - pnpm lint:tokens: PASS

- Dynamic tray status icon + startup state fixes
  - Runtime status dot overlay on tray icon (green=capturing, amber=paused)
  - icon_as_template(false) for colored rendering on macOS menu bar
  - sync_tray_state() unifies icon + menu updates across all state change paths
  - indicator_visible initialized from config.indicator.show_border
  - Overlay border starts hidden, queries backend state via IPC on mount
  - Tracking panel auto-shows on startup when config says visible
  - IPC commands (toggle_capture_pause, set_indicator_visible) sync tray icon

- Connection status in AppState + template icons + status menu
  Add server_connected, llm_connected, cli_connected AtomicBool flags to
  AppState for tracking external service connectivity. Update tray.rs to
  use TrayIconState with icon_as_template(true) for proper macOS dark/light
  mode support, and add disabled info-only connection status items to the
  tray menu showing check/cross marks per service.

- Wire connection status + get_connection_status IPC command

- Connection status indicator in tracking panel


### Changed

- Remove accent colors, add typography/spacing/motion tokens

- Remove accent CSS vars, add semantic hover colors

- Semantic variants for button/card/input/badge, remove statColorMap

- Migrate base and shell components to design tokens
  Replace hardcoded Tailwind classes with semantic design tokens across
  all UI base components and shell components:
  - Colors: dark: prefixes and text-gray-*/bg-red-*/text-white → semantic tokens
  - Motion: transition-colors/all → motion.colors/motion.all
  - Typography: font-semibold text-lg → typography.h3, font-mono text-sm → typography.mono, text-[11px] → typography.micro, text-[10px] → typography.nano
  - Icons: h-N w-N → iconSize.xs/sm/base/md/lg tokens
  - Fix broken motion.fast reference → motion.colors in Tabs

- Migrate color-critical components to design tokens
  Replace hardcoded Tailwind colors with semantic design tokens across
  10 feature components + 2 pages:

  - StatCard: remove statColorMap, use brand-signal/10 token
  - SuggestionBanner: 5-color map → semantic-info/warning/success
  - EventLog: teal/blue/purple → brand-signal, semantic-info
  - ProcessList: teal-400/blue-400 → brand-text
  - ActivityHeatmap: green gradient → brand-signal opacity scale
  - FocusWidget: colors.accent.* → brand-text, semantic-success/error
  - ErrorBoundary: red-600 → semantic-error, amber-600 → semantic-warning
  - UpdatePanel: amber-600/red-500 → semantic-warning/error, accent-teal → brand-text
  - InsightCard: accent-green/blue → semantic-success/brand-signal
  - StatisticsPanel: accent-green/red → semantic-success/error

- Migrate remaining feature components to design tokens
  Replace raw Tailwind colors, transition utilities, and motion classes with
  semantic design tokens across 10 feature components: DateRangePicker,
  DevToolbar (biome-ignore for dev-only), GuiInteractionTrack, Lightbox,
  PomodoroTimer, SegmentContextMenu, TagBadge, TagInput, TimelineScrubber,
  and TimelineView.

- Migrate all pages, settings, overlay to design tokens
  Systematic migration of 16 files:
  - Pages: Automation, DashboardDay, Focus, Privacy, Reports, Search,
    SessionReplay, Settings, Timeline, Updates
  - Settings sections: GeneralTab, OAuthConnectionPanel
  - Overlay: CoachingPopup, FocusHighlight, GoalProgressBar
  - Stories: DesignTokens.stories (remove broken colors.accent section)

  Key changes:
  - accent-teal/red/green/orange/emerald -> semantic-success/error/warning or brand-text
  - bg-teal-500/bg-black/bg-gray-* -> bg-brand/bg-surface-overlay/bg-surface-*
  - text-white -> text-content-inverse
  - dark: prefixes removed (OAuthConnectionPanel)
  - transition-colors/all/opacity -> motion.colors/all/opacity tokens
  - motion.slow -> motion.all (removed non-existent token reference)
  - Focus CATEGORY_COLORS -> brand-signal opacity variations

- Mark design system consistency spec as implemented

- Shape-based template icons replacing color dots
  Replace color-dot status overlay with shape-based template icons:
  - Active: unmodified base logo (black-on-transparent for OS tinting)
  - Paused: base logo + two vertical pause bars at bottom-right
  - Disabled: base logo + anti-aliased diagonal slash

  Add TrayIconState enum with From<bool> backward compatibility so
  existing tray.rs callers continue working until migrated.

  Remove all color constants (GREEN/AMBER), white pixel inversion,
  and circle drawing. Add draw_aa_line and draw_filled_rect helpers.


### Fixed

- Use create-dmg for professional DMG with background image
  Replace plain hdiutil create with create-dmg to apply:
  - Custom background image (dmg-background.png)
  - App icon positioned at (180, 170)
  - Applications drop link at (480, 170)
  - Volume icon from icon.icns
  - 660x400 window size

  The tauri.conf.json DMG settings were added in 79f0aa0 but never
  connected to the release pipeline which used raw hdiutil.

- Update DMG background with menu bar hint + correct arrow direction
  - Arrow points left→right (app → Applications) matching Claude DMG style
  - Added bottom hint: "After installation, ONESHIM runs in the menu bar"
  - Updated both 1x (660x400) and 2x (1320x800) Retina versions

- Resolve 6 UI/UX issues across dashboard pages

- Comprehensive UI/UX overhaul — 15+ fixes across all pages

- Resolve all lint violations — zero remaining
  - Add typography.weight.{medium,semibold,bold} and typography.family.mono
    atomic tokens to tokens.ts for ad-hoc composition
  - Add layout.commandPalette.position token for viewport-relative spacing
  - Replace 200+ hardcoded font-*, transition-*, h-N w-N, and bg-{color}-N
    classes across 30+ component/page files with design token references
  - Fix Grit plugin compilation: disable unsupported diagnostic rules,
    enforcement handled by lint-design-tokens.sh instead
  - Refine lint script: exclude DevToolbar.tsx, filter fractional widths
    from icon-size check
  - Fix ScheduleSettings quote corruption and Timeline unused variable
  - Auto-format all modified files via biome check --write

- Fix DevToolbar Vite types + update stale test counts

- Clippy TimeRangeQuery missing field + tauri config visible sync
  Add missing min_importance: None to both TimeRangeQuery construction
  sites in oneshim-web handler tests. Set main window visible: false in
  tauri.conf.json to satisfy check-config-sync (setup.rs shows it after
  init).

- Pagination total now reflects importance-filtered count
  The total count was calculated before min_importance filtering, causing
  pagination to show 26 pages when only 2 frames passed the filter.
  Now filters first, then paginates the filtered result.

- Disable macOS autocorrect/autocomplete on all text inputs
  Adds autoComplete="off", autoCorrect="off", autoCapitalize="off",
  spellCheck={false} to the base Input component and CommandPalette
